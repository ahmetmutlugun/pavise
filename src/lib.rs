...
pub fn scan_ipa(path: &Path, opts: &ScanOptions) -> Result<ScanReport> {
    ...
    let string_results: Vec<(Vec<SecretMatch>, Vec<String>, Vec<DomainInfo>, Vec<Finding>)> =
        unpacked
            ...
            .map(|f| {
                ...
                (secrets, emails, url_result.domains, url_result.findings)
            })
            .collect();
    ...
    for (secrets, emails, domains, url_findings) in string_results {
    ...
    // Deduplicate findings: same (id, first evidence value) = same finding
    ...
    // Offline: flag hardcoded IP address literals (unusual for legitimate backend communication)
    for d in &all_domains {
        if is_ip_literal(&d.domain) {
            all_findings.push(Finding {
                ...
                description: format!(
                    ...
                    pinning and may indicate C2 infrastructure or a misconfiguration.",
                    d.domain
                ),
    ...
    if let Some(ref prov) = provisioning_info {
        ...
        log.record(format!(
            "Provisioning profile: type={}, team={}, devices={}",
            prov.profile_type,
            prov.team_name.as_deref().unwrap_or("unknown"),
...
/// Deduplicate findings by (id, first evidence value). Keeps the first occurrence.
