...
toggleBtn.addEventListener("click", () => {
    if (isMobile()) {
        ...
        localStorage.setItem("pavise-sidebar", shell.classList.contains("sidebar-collapsed")
            ? "collapsed"
            : "open");
    }
});
...
async function uploadChunked(file, signal) {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        setProgress(pct, `Uploading ${formatBytes(uploaded)} / ${formatBytes(file.size)}`);
    }
    // 3. Trigger scan
...
document
    ...
    try {
        ...
        results
            ...
            if (row) {
                setTimeout(() => {
                    ...
                    row.addEventListener("animationend", () => row.classList.remove("crit-attn"), { once: true });
                }, 700 + i * 120);
            }
        });
        ...
        if (resultCard) {
            const appName = resultCard
                .querySelector(".app-name")
                ?.childNodes[0]?.textContent?.trim() ||
                "Unknown";
            const grade = resultCard
                .querySelector(".grade")
                ?.textContent?.trim() || "?";
            const score = resultCard
...
function renderHistory() {
    ...
    items.forEach((item) => {
        ...
        btn.setAttribute("aria-label", `Grade ${item.grade} — ${item.name}, scanned ${formatTimeAgo(item.ts)}`);
        btn.innerHTML = `
...
<div class="history-time">${formatTimeAgo(item.ts)}</div>
        ...
        `;
        btn.addEventListener("click", () => loadHistoryScan(item.id));
...
(function checkLandingResult() {
    let landingHtml;
    try {
        landingHtml = sessionStorage.getItem('pavise-landing-result');
    }
    catch {
        return;
    }
...
// ── Console easter egg ──
console.log("%c⬡ Pavise%c  iOS Security Analyzer\n%cParsing Mach-O since 2024. Built with Rust.\ngithub.com/ahmetmutlugun", "color:#3b82f6;font-size:15px;font-weight:900;letter-spacing:-0.02em;", "color:#8b949e;font-size:12px;font-weight:600;", "color:#4a5568;font-size:11px;line-height:1.6;");
